...
image = double(imread('user010_1.gif'));
filename = 'image10.mat';
result = double(load(filename,'-ASCII'));
drawOrientations(image,result);

function drawOrientations(img, ofield, varargin)
if (nargin==2)
    blksz = 11;
else
    blksz = varargin{1};
